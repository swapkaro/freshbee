...
## Audio update
Original instrumental tropical-racing music and river loops now accompany the race. The river increases with speed, quiets during jumps and becomes muffled underwater. Jump, boost, splash and dive effects respond to game events. M or the speaker button mutes all audio. MIX opens separate saved music/effects volume sliders. Playback starts only after a player gesture, pauses with the race or hidden tab, and restarts cleanly on replay. The portable HTML includes every audio asset, so no audio download or account is needed after saving it.

## Visual scope
This first playable build uses stylized real-time 3D, modeled geometry and shader water. It is not a prerendered image and does not reproduce the reference's photoreal console-level fidelity.
